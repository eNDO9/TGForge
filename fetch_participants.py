...
async def fetch_default_participants(client, group_name):
    ...
    try:
        ...
        # Get full channel info (to get reported members count)
        result = await client(functions.channels.GetFullChannelRequest(channel=group_name))
        reported_participants_count = result.full_chat.participants_count
        print(f"Reported members for {group_name}: {reported_participants_count}")

        # Fetch all participants (adjust limit as needed)
...
async def fetch_participants(client, group_list, method="default", start_date=None, end_date=None):
    ...
    - method: "default" uses direct API request, "messages" extracts from messages.
    Returns a unified DataFrame, total reported count (only for default), and total fetched count.
    """
    all_dfs = []
    total_reported = 0
    total_fetched = 0
    for group in group_list:
        if method == "default":
            ...
            if not df.empty:
                ...
                all_dfs.append(df)
    if not all_dfs:
        return pd.DataFrame(), total_reported, total_fetched
    unified_df = pd.concat(all_dfs, ignore_index=True)
    return unified_df, total_reported, total_fetched
